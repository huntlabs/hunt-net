...
class UnpooledTest {
    ...
    //     foreach(Entry!(byte[], Integer) e; map.entrySet()) {
    ...
    //     assertSameAndRelease(EMPTY_BUFFER, wrappedBuffer(new ByteBuf[] { Unpooled.buffer(0) }));
    ...
    //     assertSameAndRelease(EMPTY_BUFFER, copiedBuffer(new ByteBuf[] { Unpooled.buffer(0) }));
    ...
    //     ByteBuf buf = unmodifiableBuffer(Unpooled.buffer(16));
