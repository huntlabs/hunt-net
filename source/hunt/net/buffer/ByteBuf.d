 ...
 * under the License.
 */
module hunt.net.buffer.ByteBuf;
 ...
 */
static T checkNotNull(T)(T arg, string text) {
    if (arg is null) {
 ...
 */
abstract class ByteBuf : ReferenceCounted { // implements ReferenceCounted, Comparable<ByteBuf>
     ...
     */
    abstract bool isReadable(int size);
     ...
     * elements.
     */
    abstract bool isWritable(int size);
     ...
     */
    abstract int ensureWritable(int minWritableBytes, bool force);

    /**
     * Gets a bool at the specified absolute (@code index) in this buffer.
     ...
     */
    abstract bool getBoolean(int index);
     ...
     */
    abstract ByteBuf getBytes(int index, OutputStream outStream, int length);
     ...
     *         {@code index + 1} is greater than {@code this.capacity}
     */
    abstract ByteBuf setBoolean(int index, bool value);
     ...
     *         if the specified stream threw an exception during I/O
     */
    abstract int setBytes(int index, InputStream inStream, int length);
     ...
     *         if the specified stream threw an exception during I/O
     */
    abstract ByteBuf readBytes(OutputStream outStream, int length);
    ...
    /**
     * Sets the specified bool at the current {@code writerIndex}
     * and increases the {@code writerIndex} by {@code 1} in this buffer.
     * If {@code this.writableBytes} is less than {@code 1}, {@link #ensureWritable(int)}
     * will be called in an attempt to expand capacity to accommodate.
     */
    abstract ByteBuf writeBoolean(bool value);
     ...
     */
    abstract int writeBytes(InputStream inStream, int length);
     ...
     * to the backing data.
     */
    abstract bool hasMemoryAddress();
